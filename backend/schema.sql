...
CREATE TABLE IF NOT EXISTS actions (
    ...
    type TEXT NOT NULL CHECK (type IN ('light', 'avoid_light', 'sleep', 'nap', 'meal', 'hydrate', 'melatonin', 'caffeine', 'activity')),
...
CREATE POLICY "Users can view own actions" ON actions FOR SELECT USING (
    EXISTS (SELECT 1 FROM schedules 
            JOIN trips ON trips.id = schedules.trip_id 
            WHERE schedules.id = actions.schedule_id AND trips.user_id = auth.uid())
);
